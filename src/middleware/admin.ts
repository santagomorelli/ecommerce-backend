...
import { UserI, userJoiSchema } from '../models/users/users.interface';
...
export const isLoggedIn = (req: Request, res: Response, done: NextFunction) => {
  if (!req.user) return res.status(401).json({ msg: 'Usuario sin loguearse' });
...
let data: any = req.user;
 ...
 if (!data.admin || !data.admin) return res.status(401).json({ msg: 'Acceso no autorizado' });
  done();
};
