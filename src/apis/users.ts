...
class User {
  ...
  async addUser(productData: NewUserI): Promise<UserI> {
    ...
    const email: string = `${newUser}`;
    const subject: string = `${newUser.firstName} ${newUser.lastName} se ha registrado`;
    await CartAPI.createCart(newUser._id);
    await EmailService.sendEmail(`${Config.GMAIL_EMAIL}`,subject,email);
    return newUser;
